      ...
      <div className='w-full flex justify-between item-center max-w-7xl mx-auto'>
        <Link className='flex items-center gap-2' onClick={() => {setActive("");window.scrollTo(0,0)}}>
          ...
          <p className='text-white text-[18px] font-bold cursor-pointer flex'>
            Chenzhong&nbsp;
            <span className='sm:block hidden'>| Data Analyst</span></p>
        </Link>
        ...
        <div className='sm:hidden flex flex-1 justify-end items-center'>
          ...
          <div className={`${!toggle? 'hidden':'flex'} p-6 black-gradient absolute top-20 right-0 mx-4 my-2 min-w-[140px] z-10 rounded-xl`}>
            <ul className='list-non flex justify-end items-start flex-col gap-4'>
              {navLinks.map((link) => {
                return(
                <li key={link.id} className={`${active === link.title ? 'text-white':'text-secondary'} font-poppin font-medium cursor-pointer text-[16px]`}
